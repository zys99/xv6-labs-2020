...
[SYS_mkdir]   sys_mkdir,
[SYS_close]   sys_close,
[SYS_trace]   sys_trace,      // 系统调用号和处理函数关联
[SYS_sysinfo] sys_sysinfo,    // 系统调用号和处理函数关联
};
...
const char* kama_syscall_names[] = {
  ...
  [SYS_mkdir]   "mkdir",
  [SYS_close]   "close",
  [SYS_trace]   "trace",      // 系统调用号和处理函数关联
  [SYS_sysinfo] "sysinfo",    // 系统调用号和处理函数关联
  };
...
syscall(void)
{
  ...
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {   // 系统调用编号有效
    p->trapframe->a0 = syscalls[num]();   // 调用对应的处理函数, 并将返回值存在a0寄存器
