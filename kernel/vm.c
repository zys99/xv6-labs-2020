...
void kama_kvmmap_pagetable(pagetable_t pagetable) {
  // uart registers
...
kama_kvminit_newpgtbl() {
  ...
  kama_kvmmap_pagetable(pagetable);
...
// }
void
kvmmap(pagetable_t pgtbl, uint64 va, uint64 pa, uint64 sz, int perm)    // 增加参数pgtbl
{
  if(mappages(pgtbl, va, sz, pa, perm) != 0)                            // 映射pgtbl到物理地址
    panic("kvmmap");
}
